(function ($) {
  ...
  $.ImageView.prototype = {
    ...
    createOpenSeadragonInstance: function (imageUrl) {
      ...
      _this.magic = manifestor({
        manifest: this.manifest.jsonLd,
        sequence: seq,
        container: _this.elemOsd,
        perspective: 'detail',
        osdOptions:{
          uniqueID:uniqueID,
          osdID:osdID
      ...
      _this.osd = _this.magic.osd.viewer;


      _this.magic.selectViewingMode('individuals');
      _this.magic.selectPerspective('detail');

      _this.magic.selectCanvas(this.imagesList[this.currentImgIndex]['@id']);

      _this.osd.addHandler('open', function () {
        console.log('OSD IS OPENING');
      });
...
//       });
    ...
    next: function () {
      var _this = this;
       ...
       if (next < this.imagesList.length) {
         _this.magic.next();
         this.magic.selectCanvas(this.imagesList[next]['@id']);
    ...
    previous: function () {
      ...
      if (prev >= 0) {
        _this.magic.previous();
        this.magic.selectCanvas(this.imagesList[prev]['@id']);
